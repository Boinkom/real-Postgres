...
public class DogsAppServices {
    ...
    private final EntireProjectRepo entireProjectRepo;

    private static final SimpleDateFormat formatter = new SimpleDateFormat("dd-MMM-yyyy", Locale.ENGLISH);

    @Autowired
    public DogsAppServices(Repository repository, EntireProjectRepo entireProjectRepo) {
        this.repository = repository;
        this.entireProjectRepo = entireProjectRepo;
    }

    @PostMapping("/createUser/{login}")
    public String createUser(@PathVariable(value = "login") String login) {
    ...
    public String createDog(@PathVariable(value = "name") String name) {
    ...
    public String createWalk(@PathVariable(value = "userLogin") String userLogin, @PathVariable(value = "dogName") String dogName) {
    ...
    public String increaseUserBalance(@PathVariable(value = "login") String login, @PathVariable(value = "amount") double amount) {
    ...
    public String increaseUserBalanceTransactional(@PathVariable(value = "login") String login, @PathVariable(value = "amount") double amount) {
        final User user = repository.getUser(login);
        final double newBalance = user.getBalance() + amount;
        repository.updateUserBalance(login, newBalance);
        return repository.getUser(login).toString();
    ...
    public String getClosestDogs(@PathVariable(value = "login") String login, @PathVariable(value = "count") int count) {
        final User user = repository.getUser(login);
    ...
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public String createDogWithCoords(@PathVariable(value = "name") String name, @PathVariable(value = "ln") double ln, @PathVariable(value = "lat") double lat) {
    ...
    public String createDogWithCoordsSerializable(@PathVariable(value = "name") String name, @PathVariable(value = "ln") double ln, @PathVariable(value = "lat") double lat) {
        final int dogCountByCoords = repository.countDogsByCoords(ln, lat);
        if (dogCountByCoords == 0) {
            final String id = repository.createDog(name, ln, lat);
            return repository.getDog(id).toString();
    ...
    public String createDogWithCoordsAdvisoryLocked2(@PathVariable(value = "name") String name, @PathVariable(value = "ln") double ln, @PathVariable(value = "lat") double lat) {
    ...
    // Новый метод для получения топ-10 пользователей с наибольшим количеством выгулов
